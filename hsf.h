...
void print_node_type(WebServer &server, char type) {
  switch(type){
...
void webHome(WebServer &server, WebServer::ConnectionType type, char *url_tail, bool tail_complete) {
  if (server.checkCredentials(conf.user_pass)) {
    if (type == WebServer::POST) {
      ...
      ((conf.setting >> 1) & B1) ? server.printP(text_i_disabled) : server.printP(text_i_OK); server.printP(html_e_td_e_tr_tr_td);
  ...
  if (server.checkCredentials(conf.user_pass)) {
    ...
    if (type == WebServer::POST) {
      ...
      for (uint8_t i = 0; i < 21; ++i){
       ...
       /*
        for (uint8_t i = 5; i < EEPROM_MESSAGE; ++i) { 
        //  WS.print(i);
        //  WS.print(F(":"));
          WS.print(question[i]);
        //  WS.print(F(","));
       ...
       */
       ...
       time_temp = l.lval;
       server.print((char*)time_temp.formatedDateTime());
       server.printP(html_e_td_td);
       switch(value[5]){
          case 'S': // System
            ...
            switch(value[6]){
              ...
              case 'M': 
                server.printP(text_MQTT); server.print(' ');
                switch(value[7]){
                  case 'F': server.printP(text_network); server.print(' '); server.printP(text_failed); break;
                  case 'O': server.printP(text_network); server.print(' '); server.printP(text_OK); break;
                  default:  server.printP(text_undefined); break; // unknown  
                }
              break;
          ...
          case 'A': // Authentication
            ...
            switch(value[6]){
              ...
              case 'U': server.printP(text_unk); 
                //memcpy(value, &value[7], KEY_LEN);
                //formatKey(value, value); server << value;
                break;
          ...
          break;
          case 'N': // remote nodes
            print_node_type(server, value[8]);
            ...
            if (value[6] != 'r') {server.printP(text_is); server.print(' ');}
            switch(value[6]){
          ...
          case 'G': // Groups
            ...
            switch(value[6]){
              case 'F': server.printP(text_is); server.print(' '); server.printP(text_disabled); break;
              case 'S': server.printP(text_monitoring); server.print(' '); server.printP(text_started); break;
          ...
          case 'R': // Trigger
            ...
            server << (uint8_t)value[6]+1; server.printP(text_spdashsp); server << trigger[value[6]].name;
            server.print(' '); server.printP(text_activated);
          break;
          default:
  ...
  if (server.checkCredentials(conf.user_pass)) {
    ...
    if (type == WebServer::POST) {
      ...
      do {
        ...
        server.printP(html_radio_s);
        server.radioButton("9", text_1, text_On, conf.zone[webZone] >> 9 & B1);
        server.radioButton("9", text_0, text_Off, !(conf.zone[webZone] >> 9 & B1));
        server.printP(html_div_e); server.printP(html_e_td_e_tr_tr_td);
...
void webSetGroup(WebServer &server, WebServer::ConnectionType type, char *url_tail, bool tail_complete) {
  if (server.checkCredentials(conf.user_pass)) {
    if (type == WebServer::POST) {
      ...
      server.printP(html_e_th_th); server.printP(text_i_phone); server.print(' ');server.printP(text_Phone); server.print('s');
      ...
      for (uint8_t i = 0; i < ALR_GROUPS; ++i) {
        ...
        for (uint8_t ii = 0; ii < nodes; ++ii) {      
          ...
          if ((((node[ii].setting >> 1) & B1111) == i) && (node[ii].setting & B1) && (node[ii].function == 'K')) { 
            ...
            server.print(':'); server << node[ii].number; n = 1;
          }
        }
        server.printP(html_e_td_td);
        n = 0;
...
void webSetKey(WebServer &server, WebServer::ConnectionType type, char *url_tail, bool tail_complete) {
  if (server.checkCredentials(conf.user_pass)) {
    ...
    if (type == WebServer::POST) {
      ...
      server.printP(html_radio_s);
      server.radioButton("0", text_1, text_On, conf.key_setting[webKey] & B1);
      server.radioButton("0", text_0, text_Off, !(conf.key_setting[webKey] & B1));
      server.printP(html_div_e);
      server.printP(html_e_td_e_tr_tr_td);
      ...
      server.printP(html_radio_s);
      server.radioButton("5", text_1, text_Yes, conf.key_setting[webKey] >> 5 & B1);
      server.radioButton("5", text_0, text_No, !(conf.key_setting[webKey] >> 5 & B1));
      server.printP(html_div_e);
      server.printP(html_e_td_e_tr_tr_td);
      server.printP(text_Last); server.print(' '); server.printP(text_key); server.printP(html_e_td_td);
      formatKey(last_key, value); server << value; server.printP(html_e_td_e_tr_tr_td);
      server.printP(text_Group); server.printP(html_e_td_td);
      select_group(server, conf.key_setting[webKey]);
      /*
      server.printP(html_select); server << 'g'; server.printP(html_e_tag);
      for (uint8_t ii = 0; ii < ALR_GROUPS; ++ii) {
        server.printP(html_option); server << ii; 
        if ((conf.key_setting[webKey] >> 1 & B1111) == ii) { server.printP(html_selected); }
        else                                               { server.printP(html_e_tag); }
        server << ii + 1; server.printP(text_spdashsp); server << conf.group_name[ii]; server.printP(text_spdashsp); (conf.group[ii] & B1) ? server.printP(text_On) : server.printP(text_Off); server.printP(html_e_option);
      }
      server.printP(html_e_select);
  ...
  if (server.checkCredentials(conf.user_pass)) {
    ...
    if (type == WebServer::POST) {
      ...
      do {
        ...
        switch(name[0]){
          ...
          case 'n': // name
            ...
            if (_found) {
              // copy to node_name
              strncpy (node_name[_update_node].name, value, NAME_LEN);
              // prepare packet
              value[0] = 'R'; value[1] = 0;
      ...
      for (uint8_t i = 0; i < nodes; ++i) {
        ...
        server << ':' << node[i].number; server.printP(html_e_td_td);
      ...
      */
      ...
      server << ':' << node[webSens].number; server.printP(html_e_td_e_tr_tr_td);
...
void webSetMQTT(WebServer &server, WebServer::ConnectionType type, char *url_tail, bool tail_complete) {
  if (server.checkCredentials(conf.user_pass)) {
    ...
    if (type == WebServer::POST) {
      ...
      server.printP(text_MQTT); server.print(' '); server.printP(text_publish); server.printP(html_e_td_td);
      server.printP(html_radio_s);
      server.radioButton("0", text_1, text_On, (conf.mqtt >> 0) & B1);
      server.radioButton("0", text_0, text_Off, !((conf.mqtt >> 0) & B1));
      server.printP(html_div_e); server.printP(html_e_td_e_tr_tr_td); 
      ...
      server.printP(html_radio_s);
      server.radioButton("1", text_1, text_On, (conf.mqtt >> 1) & B1);
      server.radioButton("1", text_0, text_Off, !((conf.mqtt >> 1) & B1));
      server.printP(html_div_e); server.printP(html_e_td_e_tr_tr_td);
      ...
      server.printP(html_radio_s);
      server.radioButton("2", text_1, text_On, (conf.mqtt >> 2) & B1);
      server.radioButton("2", text_0, text_Off, !((conf.mqtt >> 2) & B1));
      server.printP(html_div_e); server.printP(html_e_td_e_tr_tr_td);
    ...
    if (type == WebServer::POST) {
      ...
      do {
        ...
        switch(name[0]){
          ...
          case 'r': trigger[webTrig].off_time = strtol(value, NULL, 10); break;
      ...
      for (uint8_t i = 0; i < TRIGGERS; ++i) {
        ...
        if (trigger[i].type == 'Z') {
          ...
          server.print(':'); server << trigger[i].number; server.print(':');
        ...
        server.print(':'); server << trigger[i].to_number; server.print(':');
      ...
      for (uint8_t ii = ALR_ZONES; ii < (ALR_ZONES + nodes); ++ii) {
        if (node[ii-ALR_ZONES].function == 'S') {
          ...
          server.print(':'); server << node[ii-ALR_ZONES].number; server.print(':');
          print_node_type(server, node[ii-ALR_ZONES].type);
          server.printP(html_e_option);
        }
      }
      server.printP(html_e_select); server.printP(html_e_td_e_tr_tr_td);
      ...
      server.printP(html_radio_sl);
      server.radioButton("m", text_1, text_On,  ((trigger[webTrig].setting >> 7) & B1) & !((trigger[webTrig].setting >> 8) & B1));
      server.radioButton("m", text_0, text_Off, !((trigger[webTrig].setting >> 7) & B1) & !((trigger[webTrig].setting >> 8) & B1));
      server.radioButton("m", text_2, text_Timer, !((trigger[webTrig].setting >> 7) & B1) & ((trigger[webTrig].setting >> 8) & B1));
      server.printP(html_div_e); server.printP(html_e_td_e_tr_tr_td); 
      ...
      server.radioButton("l", text_0, text_seconds, !((trigger[webTrig].setting >> 15) & B1) & !((trigger[webTrig].setting >> 14) & B1));
      server.radioButton("l", text_1, text_minutes, !((trigger[webTrig].setting >> 15) & B1) & ((trigger[webTrig].setting >> 14) & B1));
      server.radioButton("l", text_2, text_hours, ((trigger[webTrig].setting >> 15) & B1) & !((trigger[webTrig].setting >> 14) & B1));
      server.radioButton("l", text_3, text_days, ((trigger[webTrig].setting >> 15) & B1) & ((trigger[webTrig].setting >> 14) & B1));
      server.printP(html_div_e); server.printP(html_e_td_e_tr_tr_td);
      ...
      for (uint8_t ii = 0; ii < nodes; ++ii) {
        if (node[ii].function == 'I') {
          ...
          server.print(':'); server << node[ii].number; server.print(':');
          switch(node[ii].type){
            case 'D': server.printP(text_Digital); break;
            case 'A': server.printP(text_Analog); break;
            case 'F': server.printP(text_Float); break;
  ...
  if (server.checkCredentials(conf.user_pass)) {
    ...
    if (type == WebServer::POST) {
      ...
      for (uint8_t i = 0; i < TIMERS; ++i) {
        ...
        server.print(':'); server << timer[i].to_number; server.print(':');
  ...
  if (server.checkCredentials(conf.user_pass)) {
    ...
    if (type == WebServer::POST) {
      ...
      */
      ...
      server.printP(html_radio_s);
      server.radioButton("0D", text_1, text_On, conf.alerts[alert_SMS] >> 3 & B1);
      server.radioButton("0D", text_0, text_Off, !(conf.alerts[alert_SMS] >> 3 & B1));
      server.printP(html_div_e_e_td_td_radio_s);
      server.radioButton("1D", text_1, text_On, conf.alerts[1] >> 3 & B1);
      server.radioButton("1D", text_0, text_Off, !(conf.alerts[1] >> 3 & B1));
      server.printP(html_div_e_e_td_td_radio_s);
      server.radioButton("2D", text_1, text_On, conf.alerts[2] >> 3 & B1);
      server.radioButton("2D", text_0, text_Off, !(conf.alerts[2] >> 3 & B1));
      server.printP(html_div_e); server.printP(html_e_td_e_tr_tr_td);
